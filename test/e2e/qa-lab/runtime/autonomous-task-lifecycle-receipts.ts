...
function requireOwnerReceipt(result: AuditRunInspectResult, owner: string) {
  const receipt = result.decisions.find((candidate) => candidate.source.owner === owner);
  if (
    !receipt ||
    receipt.enforcement.coverageState !== "attribution-only" ||
    receipt.decision.outcome !== "not-applicable"
  ) {
    throw new Error(`inspection omitted exact attribution-only ${owner} receipt`);
  }
  return receipt;
}
...
async function inspectExecution(params: {
  ...
  for (const owner of params.owners) {
    requireOwnerReceipt(json, owner);
  }
  ...
  for (const owner of params.owners) {
    if (!human.includes(owner)) {
      throw new Error(`human inspection omitted ${owner}`);
    }
  }
...
async function runProof(options: ProducerOptions): Promise<string> {
  ...
  try {
    ...
    const cronInspection = await inspectExecution({
      ...
      owners: ["cron_run_receipts", "task_runs"],
    ...
    requireOwnerReceipt(cronCursorPage, "cron_run_receipts");
