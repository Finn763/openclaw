...
Run on this branch (round-7 proof commit `0cb6f19406a`):
...
```text
  ...
  enqueueFollowupRun: accepted=true, queue depth 1
  scheduleFollowupDrain → recording transport captures exactly 1 dispatch
    dispatched[0].prompt = "round-7 isolated-gateway inbound"
```
...
## Round-7 follow-up: P2 retained-singleton backfill + real-queue reply drain

The round-7 verdict also raised one actionable finding: `resolveGlobalSingleton` can return a reply-registry instance created before `sourceTurnByKey` existed, and the next admitted source turn then throws on `.set` in `bindSourceTurnId()`, failing the reply. Both items are closed at head `cdb4850fa40` (`e8f3288b38d` + `cdb4850fa40` on top of the round-7 proof commit `0cb6f19406a`).

**P2 retained-singleton backfill.** `src/auto-reply/reply/reply-run-registry.state.ts` now backfills `replyRunState.sourceTurnByKey ??= new Map();` right beside the existing `followupAdmissionBarriersByKey` / `successorAdmissionBarriersByKey` backfills, matching the established upgrade pattern. New coverage: `src/auto-reply/reply/reply-run-registry.retained-singleton.test.ts` plants a pre-change registry instance (every map the old module version knew, no `sourceTurnByKey`) on `globalThis`, reloads the module graph, and asserts `bindSourceTurnId` / `getSourceTurnId` work through the retained object.

Red → green, same host, same harness:

```text
FAIL  reply-run-registry.retained-singleton.test.ts > backfills sourceTurnByKey when the retained singleton predates the field
      TypeError: Cannot read properties of undefined (reading 'set')
      ❯ Object.bindSourceTurnId src/auto-reply/reply/reply-run-registry.registry.ts:122
FAIL  reply-run-registry.retained-singleton.test.ts > keeps the retained singleton's pre-existing maps usable after backfill
      (same TypeError)
Tests  2 failed (2)

→ with the one-line backfill:

✓ reply-run-registry.retained-singleton.test.ts (2 passed)
```

**Proof upgrade: the observable reply is drained by the real queue.** The seam mock in the isolated-gateway proof previously authored the reply itself. It now records the projected inbound dispatch and then routes the rejected inbound through the REAL follow-up queue machinery — `enqueueFollowupRun` + `scheduleFollowupDrain`, both real implementations — into a recording runner that stands in for the transport. The proof asserts `enqueueFollowupRun` accepted with depth 1, the drain dispatched exactly once, and the captured outbound reply carries the inbound text. Combined with the zero-steer-enqueue spy and the wire assertions, the evidence chain is: production `chat.send` handler → real admission fence → zero steer enqueues → exactly one follow-up dispatch → exactly one observable reply through the real queue.

**Suite state at head `cdb4850fa40` (this branch, same in-repo harnesses CI uses):**

```text
reply-run-registry.retained-singleton.test.ts           2 passed (2)
reply-run-registry.test.ts                             87 passed (87)
chat-send-message-injection.test.ts + real-dispatch   13 passed (13)
chat-send-message-injection.isolated-gateway.test.ts    3 passed (3)
restart-recovery-receipt.test.ts                       11 passed + 8 EPERM teardown noise (Windows host, pre-existing; CI runs Linux)
```

`oxfmt --check` and oxlint are clean on all changed files.
