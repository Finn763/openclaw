...
**Real gateway dispatch trace — mechanism and constraints.** The requested redacted ephemeral-gateway trace (one visible follow-up reply with zero active-run enqueues, driven through real chat dispatch) cannot be produced on this host, and we state the constraints precisely rather than fabricate one:

1. **No gateway deployment exists here.** `~/.openclaw` contains no gateway config and no channel credentials; this is an isolated build/test host, not a running gateway.
2. **No channel credentials.** No Telegram/WhatsApp bot tokens or account bindings exist on this host, and the environment carries no `OPENCLAW_*` channel secrets. The in-repo live-lane harness (`extensions/qa-lab` `startQaLiveLaneGateway`) requires real `OPENCLAW_QA_TELEGRAM_*` driver/SUT bot tokens and live model-provider credentials to drive a real inbound.
3. **No live model credentials.** An "observed after-fix reply" requires a real agent run completing a terminal send, which requires live LLM provider credentials — also absent here.

The strongest real-dispatch evidence available without production credentials is therefore: (a) the ClawSweeper live verification of this branch's gateway suite (8/8 PASS on the reviewed head), (b) the front-fence red→green runs above on the same in-repo harness CI uses, and (c) the runner e2e suite. A maintainer with a live gateway can obtain the requested trace by running any real channel inbound against a session whose active source turn is fail-closed and observing the follow-up queue; the deterministic admission decision is the same code path the unit suites lock.
